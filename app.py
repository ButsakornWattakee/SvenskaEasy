...
# If logged in, refresh user data from MongoDB to ensure real-time update/deletion detection
...
if st.session_state.current_page == "แดชบอร์ดผู้ดูแลระบบ":
    ...
    if users_list:
        ...
        # Add Delete User feature for admin
        st.markdown("### 🗑️ จัดการลบผู้เรียนออกจากระบบ (Delete User)")
        deletable_users = [u["username"] for u in users_list if u["username"] != "admin"]
        
        if deletable_users:
            col_select, col_btn = st.columns([3, 1])
            with col_select:
                user_to_delete = st.selectbox("เลือกผู้ใช้ที่ต้องการลบ:", deletable_users, key="user_to_delete_select", label_visibility="collapsed")
            with col_btn:
                if st.button("🚨 ลบผู้ใช้ที่เลือก", key="delete_user_btn", type="primary", use_container_width=True):
                    ...
                    if success:
                        ...
                        st.error("เกิดข้อผิดพลาดในการลบผู้ใช้")
        else:
            st.info("ไม่มีสมาชิกอื่นที่สามารถลบได้ในขณะนี้")
    else:
