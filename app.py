...
st.set_page_config(
    page_title="Learn Swedish - เรียนภาษาสวีเดนสำหรับคนไทย",
    layout="wide",
    ...
    with col2:
        st.markdown(
            ...
            <div style='text-align: center; margin-top: 50px; margin-bottom: 20px;'>
                <h1 style='font-size: 3rem; margin-bottom: 0;'>LearnSwedish</h1>
                <p style='font-size: 1.2rem; opacity: 0.8;'>เรียนภาษาสวีเดนเพื่อคนไทย</p>
            </div>
            """, 
...
# Sidebar Navigation
with st.sidebar:
    st.markdown('<div class="sidebar-title"><h1>LearnSwedish</h1><p>เรียนภาษาสวีเดนเพื่อคนไทย</p></div>', unsafe_allow_html=True)
...
# ----------------- 3. QUIZZES PAGE -----------------
    ...
    with tab_choice:
        ...
        if submit_button:
            ...
            prev_score = st.session_state.quiz_scores.get(l_id, 0)
            if score_percent > prev_score:
                st.session_state.quiz_scores[l_id] = score_percent
            ...
            with st.form(key=match_form_key):
                ...
                with col_c1:
                    ...
                    st.markdown(
                        ...
                        <div class="match-card">
                            <span style="font-size: 1.4rem; font-weight: bold;">{thai_1}</span>
                        </div>
                        """,
                        unsafe_allow_html=True
                    )
                ...
                with col_c2:
                    ...
                    st.markdown(
                        f"""
                        <div class="match-card">
                            <span style="font-size: 1.4rem; font-weight: bold;">{thai_2}</span>
                        </div>
                        """,
                        unsafe_allow_html=True
                    )
                ...
                with col_c3:
                    ...
                    st.markdown(
                        f"""
                        <div class="match-card">
                            <span style="font-size: 1.4rem; font-weight: bold;">{thai_3}</span>
                        </div>
                        """,
                        unsafe_allow_html=True
                    )
            ...
            if submit_match_button:
                ...
                for i, item in enumerate(mp_list):
                    ...
                    if selected == "เลือกคำศัพท์...":
                        st.markdown(
                            f"""
                            <div class="quiz-feedback-error" style="border-left: 5px solid #6c757d;">
